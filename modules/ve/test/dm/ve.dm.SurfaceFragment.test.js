...
QUnit.test( 'removeContent', 3, function ( assert ) {
	var doc = new ve.dm.Document( ve.copyArray( ve.dm.example.data ) ),
		surface = new ve.dm.Surface( doc ),
	...
	assert.deepEqual(
		doc.data.internal,
		{ 'changed': { 'childrenRemoved': 3 } },
		'removing sets change marker on document for removed top-level nodes'
	);
	assert.deepEqual(
		fragment.getRange(),
		new ve.Range( 1, 1 ),
		...
		[
			{
				...
				'internal': { 'changed': { 'new': 1 } }
			},
			{ 'type': 'listItem', 'internal': { 'changed': { 'new': 1 } } },
			{ 'type': 'paragraph' },
			'l',
			{ 'type': '/paragraph' },
			{ 'type': '/listItem' },
