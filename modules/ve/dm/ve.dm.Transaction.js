 ...
 * that effected the changes. Additionally, the Document object tracks change markers for the
 * document node; in the change markers map in the Transaction, the document's change markers are
 * at the virtual offset -1.
 *
 ...
 * element marked 'new' is removed, this isn't tracked, because the removed element never existed
 * in the original document, so it's not a change relative to the original state of the document.
 ...
 *
 * Marker types:
 * - 'new': This element was newly created and did not exist in the original document
 ...
 * - 'childrenRemoved': Some of this element's children were removed
 *
...
ve.dm.Transaction.prototype.setChangeMarker = function ( offset, marker, increment ) {
	...
	if ( this.changeMarkers[offset]['new'] ) {
		// Can't set any other markers on a 'new' element
		return;
	}
	if ( marker === 'new' ) {
		// Clear other markers prior to setting 'new'
		this.changeMarkers[offset] = {};
	}
